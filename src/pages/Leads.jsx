    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-[#FDFBF7] dark:bg-[#1E1B17] border border-[#E0D9CE] dark:border-[#2A2520] rounded-xl w-full max-w-md p-6">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-sm font-semibold text-[#1A1814] dark:text-[#EDE7DC]">Editar lead</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-xs font-medium text-[#1A1814] dark:text-[#EDE7DC] mb-1.5 uppercase tracking-wide">Nombre *</label>
          ...
          <div>
            <label className="block text-xs font-medium text-[#1A1814] dark:text-[#EDE7DC] mb-1.5 uppercase tracking-wide">Teléfono</label>
            <input type="tel" value={form.phone} onChange={e => set('phone', e.target.value)}
              className="w-full px-3 py-2 border border-[#D9D9D9] text-sm text-[#1A1814] dark:text-[#EDE7DC] rounded-xl outline-none focus:border-[#1A1814] transition-colors" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-[#1A1814] dark:text-[#EDE7DC] mb-1.5 uppercase tracking-wide">Tipo de evento</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-[#1A1814] dark:text-[#EDE7DC] mb-1.5 uppercase tracking-wide">Canal</label>
            ...
            <div>
              <label className="block text-xs font-medium text-[#1A1814] dark:text-[#EDE7DC] mb-1.5 uppercase tracking-wide">Monto cotizado ($)</label>
      ...
      <Draggable draggableId={lead.id} index={index}>
        {(provided, snapshot) => (
          <div
            ...
            className={`
              bg-[#FDFBF7] dark:bg-[#1E1B17] border border-[#E0D9CE] dark:border-[#2A2520] border-l-4 ${STAGE_ACCENT[lead.stage]}
          ...
          >
            <div className="flex items-start justify-between gap-1 mb-1">
              <div className="text-sm font-medium text-[#1A1814] dark:text-[#EDE7DC] leading-tight">{lead.clients?.name}</div>
              <div className="flex items-center gap-1 flex-shrink-0">
                {!confirmDelete ? (
                  <>
                    <button onClick={e => { e.stopPropagation(); setShowEdit(true) }}
                      className="text-[#CCC] dark:text-[#4A4440] hover:text-[#1A1814] dark:text-[#EDE7DC] transition-colors mt-0.5" title="Editar">
                      <Pencil size={11} />
                    </button>
                    <button onClick={e => { e.stopPropagation(); navigate(`/clientes/${lead.client_id}`) }}
                      className="text-[#CCC] dark:text-[#4A4440] hover:text-[#1A1814] dark:text-[#EDE7DC] transition-colors mt-0.5" title="Ver ficha">
            ...
            {lead.amount_quoted && (
              <div className="text-xs font-medium text-[#1A1814] dark:text-[#EDE7DC]">{formatUSD(lead.amount_quoted)}</div>
            )}
          </div>
        )}
      </Draggable>
  ...
  return (
    <div className="flex flex-col w-56 flex-shrink-0">
      <div className="flex items-center justify-between mb-3">
        <span className="text-xs font-semibold text-[#1A1814] dark:text-[#EDE7DC] uppercase tracking-wider">
          {stage.label}
        </span>
...
function NewLeadModal({ onClose, onCreated }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-[#FDFBF7] dark:bg-[#1E1B17] border border-[#E0D9CE] dark:border-[#2A2520] rounded-xl w-full max-w-md p-6">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-sm font-semibold text-[#1A1814] dark:text-[#EDE7DC]">Nuevo lead</h2>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl font-semibold text-[#1A1814] dark:text-[#EDE7DC]">Pipeline de leads</h1>
        <button
