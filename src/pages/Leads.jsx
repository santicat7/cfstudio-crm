    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-[#FDFBF7] dark:bg-[#232019] border border-[#E0D9CE] dark:border-[#2E2923] rounded-xl w-full max-w-md p-6">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-sm font-semibold text-[#1A1814] dark:text-[#C8C0B4]">Editar lead</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-xs font-medium text-[#1A1814] dark:text-[#C8C0B4] mb-1.5 uppercase tracking-wide">Nombre *</label>
          ...
          <div>
            <label className="block text-xs font-medium text-[#1A1814] dark:text-[#C8C0B4] mb-1.5 uppercase tracking-wide">Teléfono</label>
            <input type="tel" value={form.phone} onChange={e => set('phone', e.target.value)}
              className="w-full px-3 py-2 border border-[#D9D9D9] text-sm text-[#1A1814] dark:text-[#C8C0B4] rounded-xl outline-none focus:border-[#1A1814] transition-colors" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-[#1A1814] dark:text-[#C8C0B4] mb-1.5 uppercase tracking-wide">Tipo de evento</label>
            ...
            <div>
              <label className="block text-xs font-medium text-[#1A1814] dark:text-[#C8C0B4] mb-1.5 uppercase tracking-wide">Fecha</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-[#1A1814] dark:text-[#C8C0B4] mb-1.5 uppercase tracking-wide">Canal</label>
            ...
            <div>
              <label className="block text-xs font-medium text-[#1A1814] dark:text-[#C8C0B4] mb-1.5 uppercase tracking-wide">Monto cotizado ($)</label>
      ...
      <Draggable draggableId={lead.id} index={index}>
        {(provided, snapshot) => (
          <div
            ...
            className={`
              bg-[#FDFBF7] dark:bg-[#232019] border border-[#E0D9CE] dark:border-[#2E2923] border-l-4 ${STAGE_ACCENT[lead.stage]}
          ...
          >
            <div className="flex items-start justify-between gap-1 mb-1">
              <div className="text-sm font-medium text-[#1A1814] dark:text-[#C8C0B4] leading-tight">{lead.clients?.name}</div>
              <div className="flex items-center gap-1 flex-shrink-0">
                {!confirmDelete ? (
                  <>
                    <button onClick={e => { e.stopPropagation(); setShowEdit(true) }}
                      className="text-[#CCC] dark:text-[#4A4440] hover:text-[#1A1814] dark:text-[#C8C0B4] transition-colors mt-0.5" title="Editar">
                      <Pencil size={11} />
                    </button>
                    <button onClick={e => { e.stopPropagation(); navigate(`/clientes/${lead.client_id}`) }}
                      className="text-[#CCC] dark:text-[#4A4440] hover:text-[#1A1814] dark:text-[#C8C0B4] transition-colors mt-0.5" title="Ver ficha">
                      <ExternalLink size={11} />
                    </button>
                    <button onClick={e => { e.stopPropagation(); setConfirmDelete(true) }}
                      className="text-[#CCC] dark:text-[#4A4440] hover:text-red-400 transition-colors mt-0.5">
                      <X size={12} />
                    </button>
                  </>
                ) : (
                  <div className="flex gap-1 flex-shrink-0">
                    ...
                    <button onClick={e => { e.stopPropagation(); setConfirmDelete(false) }} className="text-[10px] text-[#888] dark:text-[#7A7068]">No</button>
                  </div>
                )}
              </div>
            </div>
            <div className="text-xs text-[#888] dark:text-[#7A7068] mb-1">{lead.clients?.event_type}</div>
            {lead.clients?.event_date && (
              <div className="text-xs text-[#888] dark:text-[#7A7068] mb-2">
                {format(new Date(lead.clients.event_date + 'T12:00:00'), "d MMM yyyy", { locale: es })}
              </div>
            )}
            {lead.amount_quoted && (
              <div className="text-xs font-medium text-[#1A1814] dark:text-[#C8C0B4]">{formatUSD(lead.amount_quoted)}</div>
            )}
          </div>
        )}
      </Draggable>
  ...
  return (
    <div className="flex flex-col w-56 flex-shrink-0">
      <div className="flex items-center justify-between mb-3">
        <span className="text-xs font-semibold text-[#1A1814] dark:text-[#C8C0B4] uppercase tracking-wider">
          {stage.label}
        </span>
...
function NewLeadModal({ onClose, onCreated }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-[#FDFBF7] dark:bg-[#232019] border border-[#E0D9CE] dark:border-[#2E2923] rounded-xl w-full max-w-md p-6">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-sm font-semibold text-[#1A1814] dark:text-[#C8C0B4]">Nuevo lead</h2>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl font-semibold text-[#1A1814] dark:text-[#C8C0B4]">Pipeline de leads</h1>
        <button
      ...
      {loading ? (
        <div className="text-sm text-[#888] dark:text-[#7A7068] py-12 text-center">Cargando leads...</div>
      ) : (
