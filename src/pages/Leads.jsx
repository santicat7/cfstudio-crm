    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-card border border-line rounded-xl w-full max-w-md p-6">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-sm font-semibold text-body">Editar lead</h2>
          <button onClick={onClose} className="text-muted hover:text-body transition-colors"><X size={16} /></button>
        </div>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-xs font-medium text-body mb-1.5 uppercase tracking-wide">Nombre *</label>
          ...
          <div>
            <label className="block text-xs font-medium text-body mb-1.5 uppercase tracking-wide">Teléfono</label>
            <input type="tel" value={form.phone} onChange={e => set('phone', e.target.value)}
              className="w-full px-3 py-2 border border-line text-sm text-body rounded-xl outline-none focus:border-strong transition-colors" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-body mb-1.5 uppercase tracking-wide">Tipo de evento</label>
            ...
            <div>
              <label className="block text-xs font-medium text-body mb-1.5 uppercase tracking-wide">Fecha</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-body mb-1.5 uppercase tracking-wide">Canal</label>
            ...
            <div>
              <label className="block text-xs font-medium text-body mb-1.5 uppercase tracking-wide">Monto cotizado ($)</label>
      ...
      <Draggable draggableId={lead.id} index={index}>
        {(provided, snapshot) => (
          <div
            ...
            className={`
              bg-card border border-line border-l-4 ${STAGE_ACCENT[lead.stage]}
          ...
          >
            <div className="flex items-start justify-between gap-1 mb-1">
              <div className="text-sm font-medium text-body leading-tight">{lead.clients?.name}</div>
              <div className="flex items-center gap-1 flex-shrink-0">
                {!confirmDelete ? (
                  <>
                    <button onClick={e => { e.stopPropagation(); setShowEdit(true) }}
                      className="text-dim hover:text-body transition-colors mt-0.5" title="Editar">
                      <Pencil size={11} />
                    </button>
                    <button onClick={e => { e.stopPropagation(); navigate(`/clientes/${lead.client_id}`) }}
                      className="text-dim hover:text-body transition-colors mt-0.5" title="Ver ficha">
                      <ExternalLink size={11} />
                    </button>
                    <button onClick={e => { e.stopPropagation(); setConfirmDelete(true) }}
                      className="text-dim hover:text-red-400 transition-colors mt-0.5">
                      <X size={12} />
                    </button>
                  </>
                ) : (
                  <div className="flex gap-1 flex-shrink-0">
                    ...
                    <button onClick={e => { e.stopPropagation(); setConfirmDelete(false) }} className="text-[10px] text-muted">No</button>
                  </div>
                )}
              </div>
            </div>
            <div className="text-xs text-muted mb-1">{lead.clients?.event_type}</div>
            ...
            {lead.amount_quoted && (
              <div className="text-xs font-medium text-body">{formatUSD(lead.amount_quoted)}</div>
            )}
          </div>
        )}
      </Draggable>
        ...
        {(provided, snapshot) => (
          <div
            ...
            className={`flex-1 min-h-[200px] rounded-xl p-1 transition-colors ${
              snapshot.isDraggingOver ? 'bg-subtle' : 'bg-page'
            }`}
          >
            {leads.map((lead, idx) => (
...
function NewLeadModal({ onClose, onCreated }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-card border border-line rounded-xl w-full max-w-md p-6">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-sm font-semibold text-body">Nuevo lead</h2>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl font-semibold text-body">Pipeline de leads</h1>
        <button
      ...
      {loading ? (
        <div className="text-sm text-muted py-12 text-center">Cargando leads...</div>
      ) : (
