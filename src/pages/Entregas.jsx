...
// ─── Constantes ───────────────────────────────────────────────────────────────
const STATUSES = [
  { value: 'sin_editar', label: 'Sin editar',  badge: 'bg-[#EDE7DC] text-[#555] dark:text-[#A8A098]' },
  ...
  return (
    <div ref={ref} className="fixed z-50 bg-[#FDFBF7] dark:bg-[#232019] border border-[#E0D9CE] dark:border-[#2E2923] rounded-xl w-72 p-4"
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <button onClick={onClose} className="text-[#CCC] dark:text-[#4A4440] hover:text-[#888] dark:text-[#7A7068] transition-colors"><X size={13} /></button>
      </div>
            ...
            {STATUSES.map(s => (
              <button key={s.value} onClick={() => setStatus(s.value)}
                className={`text-xs px-2 py-1.5 rounded-xl border transition-colors text-left ${
                  status === s.value ? 'border-[#1A1814] bg-[#1A1814] text-white' : 'border-[#D9D9D9] text-[#666] dark:text-[#998E88] hover:border-[#888]'
                }`}>
                {s.label}
              </button>
            ))}
...
function DeliveryRow({ delivery, onUpdated }) {
  ...
  return (
    <div className={`flex items-center gap-4 px-5 py-3.5 border-b border-[#E0D9CE] dark:border-[#2E2923] last:border-0 ${overdue ? 'bg-red-50' : ''}`}>
      ...
      <div className={`text-xs flex-shrink-0 w-24 text-right ${overdue ? 'text-red-600 font-medium' : 'text-[#888] dark:text-[#7A7068]'}`}>
  ...
  return (
    <button onClick={handle}
      className={`flex items-center gap-1.5 text-xs px-2.5 py-1.5 rounded-xl border font-medium transition-all ${
        copied ? 'bg-green-600 border-green-600 text-white' : 'border-[#D9D9D9] text-[#555] dark:text-[#A8A098] hover:border-[#1A1814]'
      }`}>
      ...
      <div className="flex items-start justify-between gap-4 mb-3">
        ...
        <div className="flex flex-col gap-1.5 flex-shrink-0 items-end">
          {daysSince !== null && daysSince < 3 && (
            <span className="text-xs text-[#AAA] dark:text-[#5A5450]">
          ...
          {!show3 && daysSince === null && (
            <span className="text-xs text-[#AAA] dark:text-[#5A5450]">Sin fecha de entrega</span>
          )}
          <button
            onClick={async () => {
              await supabase.from('deliveries').delete().eq('id', delivery.id)
        ...
        <>
          <div className="bg-[#FDFBF7] dark:bg-[#232019] border border-[#E0D9CE] dark:border-[#2E2923] rounded-xl overflow-hidden">
            ...
            {loading ? (
              ...
              <div className="px-5 py-10 text-center text-sm text-[#AAA] dark:text-[#5A5450]">
          ...
          <p className="text-xs text-[#AAA] dark:text-[#5A5450] mt-3">
