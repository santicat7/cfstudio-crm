...
// ─── Constantes ───────────────────────────────────────────────────────────────
const STATUSES = [
  { value: 'sin_editar', label: 'Sin editar',  badge: 'bg-subtle text-soft' },
...
function EditPopover({ delivery, anchor, onClose, onSaved }) {
  ...
  return (
    <div ref={ref} className="fixed z-50 bg-card border border-line rounded-xl w-72 p-4"
      style={{ top: pos.top, left: pos.left, boxShadow: '0 4px 24px rgba(0,0,0,0.10)' }}>
      <div className="flex items-center justify-between mb-3">
        <span className="text-xs font-semibold uppercase tracking-wider text-gold">Editar entrega</span>
        <button onClick={onClose} className="text-dim hover:text-muted transition-colors"><X size={13} /></button>
      </div>
      <div className="space-y-3">
        <div>
          <label className="block text-xs font-medium text-body mb-1.5 uppercase tracking-wide">Estado</label>
          <div className="grid grid-cols-2 gap-1.5">
            {STATUSES.map(s => (
              <button key={s.value} onClick={() => setStatus(s.value)}
                className={`text-xs px-2 py-1.5 rounded-xl border transition-colors text-left ${
                  status === s.value ? 'border-strong bg-ink text-white' : 'border-line text-soft hover:border-line'
                }`}>
                {s.label}
              </button>
            ))}
        ...
        <div>
          <label className="block text-xs font-medium text-body mb-1.5 uppercase tracking-wide">Link galería</label>
          <input type="url" value={galleryUrl} onChange={e => setGalleryUrl(e.target.value)}
            placeholder="Drive, WeTransfer..."
            className="w-full px-2.5 py-1.5 border border-line text-xs text-body rounded-xl outline-none focus:border-strong transition-colors" />
        </div>
        <div>
          <label className="block text-xs font-medium text-body mb-1.5 uppercase tracking-wide">Fecha prometida</label>
          <input type="date" value={promisedAt} onChange={e => setPromisedAt(e.target.value)}
            className="w-full px-2.5 py-1.5 border border-line text-xs text-body rounded-xl outline-none focus:border-strong transition-colors" />
        </div>
        <button onClick={save} disabled={saving}
          className="w-full bg-ink text-white text-xs py-2 rounded-xl hover:bg-ink transition-colors disabled:opacity-50">
...
function DeliveryRow({ delivery, onUpdated }) {
  ...
  return (
    <div className={`flex items-center gap-4 px-5 py-3.5 border-b border-line last:border-0 ${overdue ? 'bg-red-50' : ''}`}>
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <span className="text-sm font-medium text-body">{delivery.clients?.name}</span>
          {overdue && <span className="text-[10px] font-medium text-red-600 bg-red-100 px-1.5 py-0.5 rounded-xl">Atrasado</span>}
        </div>
        <div className="text-xs text-muted mt-0.5">{delivery.clients?.event_type}</div>
      </div>
      ...
      <div className={`text-xs flex-shrink-0 w-24 text-right ${overdue ? 'text-red-600 font-medium' : 'text-muted'}`}>
      ...
      <div className="w-8 flex-shrink-0 flex justify-end">
        {delivery.gallery_url
          ? <a href={delivery.gallery_url} target="_blank" rel="noopener noreferrer"
              className="text-muted hover:text-body transition-colors" onClick={e => e.stopPropagation()}>
              <ExternalLink size={14} />
            </a>
          : <span className="text-dim"><ExternalLink size={14} /></span>
        }
      </div>
      <button
        onClick={async () => {
  ...
  return (
    <button onClick={handle}
      className={`flex items-center gap-1.5 text-xs px-2.5 py-1.5 rounded-xl border font-medium transition-all ${
        copied ? 'bg-green-600 border-green-600 text-white' : 'border-line text-soft hover:border-strong'
      }`}>
  ...
  return (
    <div className="px-5 py-4 border-b border-line last:border-0">
      <div className="flex items-start justify-between gap-4 mb-3">
        ...
        <div className="flex flex-col gap-1.5 flex-shrink-0 items-end">
          ...
          {!show3 && daysSince === null && (
            <span className="text-xs text-faint">Sin fecha de entrega</span>
          )}
          <button
            onClick={async () => {
              await supabase.from('deliveries').delete().eq('id', delivery.id)
            ...
            className="text-dim hover:text-red-400 transition-colors mt-1"
            title="Eliminar entrega"
          >
            <Trash2 size={12} />
          </button>
      ...
      <input
        ...
        className="w-full px-3 py-1.5 border border-line text-xs text-body rounded-xl outline-none focus:border-line transition-colors placeholder:text-dim"
      />
    </div>
  )
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-card border border-line rounded-xl w-full max-w-sm p-6">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-sm font-semibold text-body">Nueva entrega</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-xs font-medium text-body mb-1.5 uppercase tracking-wide">
              Fecha límite <span className="normal-case font-normal text-faint">(auto: evento +30 días)</span>
            </label>
            <input type="date" value={promisedAt} onChange={e => setPromisedAt(e.target.value)}
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-semibold text-body">Entregas</h1>
      ...
      <div className="flex gap-1 mb-5">
        <button onClick={() => setTab('en_curso')}
          className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-xl font-medium transition-colors ${
            tab === 'en_curso' ? 'bg-ink text-white' : 'bg-card border border-line text-soft hover:border-strong'
          }`}>
          En curso
          {enCurso.length > 0 && (
            <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${tab === 'en_curso' ? 'bg-card' : 'bg-subtle'}`}>
              {enCurso.length}
            </span>
          )}
        </button>
        <button onClick={() => setTab('seguimiento')}
          className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-xl font-medium transition-colors ${
            tab === 'seguimiento' ? 'bg-ink text-white' : 'bg-card border border-line text-soft hover:border-strong'
          }`}>
          <MessageCircle size={11} />
          Seguimiento post-entrega
          {entregadas.length > 0 && (
            <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${tab === 'seguimiento' ? 'bg-card' : 'bg-subtle'}`}>
              {entregadas.length}
            </span>
          )}
        </button>
      ...
      {tab === 'en_curso' && (
        <>
          <div className="bg-card border border-line rounded-xl overflow-hidden">
            <div className="flex items-center gap-4 px-5 py-2.5 border-b border-line bg-page">
              <div className="flex-1 text-xs font-semibold uppercase tracking-wider text-gold">Cliente</div>
              <div className="flex-shrink-0 w-28 text-xs font-semibold uppercase tracking-wider text-gold">Estado</div>
              <div className="flex-shrink-0 w-24 text-right text-xs font-semibold uppercase tracking-wider text-gold">Entrega</div>
