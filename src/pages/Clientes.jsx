  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl font-semibold text-body">Clientes</h1>
        <div className="flex gap-2">
          <button
            onClick={exportCSV}
            className="flex items-center gap-1.5 border border-line text-sm text-soft px-4 py-2 rounded-xl hover:border-strong hover:text-body transition-colors"
          >
            <Download size={14} />
            Exportar CSV
          </button>
      ...
      <div className="flex gap-2 mb-4">
        <div className="relative flex-1 max-w-xs">
          ...
          <input
            ...
            className="w-full pl-8 pr-3 py-2 border border-line text-sm text-body rounded-xl outline-none focus:border-strong transition-colors"
          />
        </div>
        <select
          value={filterType}
          onChange={e => setFilterType(e.target.value)}
          className="px-3 py-2 border border-line text-sm text-body rounded-xl outline-none focus:border-strong bg-card transition-colors"
        >
      ...
      <div className="bg-card border border-line rounded-xl overflow-hidden">
        {/* Header */}
        <div className="grid grid-cols-[2fr_1fr_1fr_1fr_1fr_auto] gap-4 px-5 py-2.5 border-b border-line bg-page">
          ...
          filtered.map(c => {
            ...
            return (
              <div
                ...
                className="grid grid-cols-[2fr_1fr_1fr_1fr_1fr_auto] gap-4 px-5 py-3.5 border-b border-line last:border-0 hover:bg-page cursor-pointer transition-colors items-center"
              >
                <div className="flex items-center gap-2">
                  <span className="text-sm font-medium text-body">{c.name}</span>
                  {c.visit_count > 1 && (() => {
                ...
                <div className="text-sm text-soft">{c.event_type || '—'}</div>
                <div className="text-sm text-soft">{formatDate(c.event_date)}</div>
                <div className="text-sm text-soft">{c.package || '—'}</div>
                <div>
                  {delivery ? (
